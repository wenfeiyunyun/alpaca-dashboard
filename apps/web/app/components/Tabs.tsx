...
export function UnifiedPositionsOrdersTab({ positions, orders }: UnifiedTabProps) {
  ...
  return (
    <div>
      <h3 style={{ marginTop: 0, marginBottom: '15px', color: '#58a6ff' }}>Positions & Orders</h3>
      
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
        {/* Left: Positions */}
        <div style={{ padding: '12px', background: '#161b22', borderRadius: '8px' }}>
          <h4 style={{ marginTop: 0, marginBottom: '8px', color: '#3fb950', fontSize: '12px' }}>Open Positions</h4>
          {positions.length === 0 ? (
            <p style={{ color: '#8b949e', fontSize: '11px' }}>No open positions</p>
          ) : (
              ...
              <tbody>
                {positions.map((p, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #21262d' }}>
                    ...
                    <td style={{ padding: '2px', color: parseFloat(p.unrealized_pl) >= 0 ? '#3fb950' : '#f85149' }}>
                      {p.unrealized_pl}
                    </td>
                  </tr>
                ))}
        ...
        {/* Right: Orders */}
        <div style={{ padding: '12px', background: '#161b22', borderRadius: '8px' }}>
          <h4 style={{ marginTop: 0, marginBottom: '8px', color: '#58a6ff', fontSize: '12px' }}>Open Orders</h4>
          {orders.length === 0 ? (
            <p style={{ color: '#8b949e', fontSize: '11px' }}>No open orders</p>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #21262d' }}>
...
export function TradeTab({ onSubmit, loading, message }: TradeProps) {
  ...
  return (
    <div style={{ maxWidth: '400px' }}>
      <h3 style={{ marginTop: 0, color: '#58a6ff' }}>Place Order</h3>
      
      <div style={{ marginBottom: '12px' }}>
        <label style={{ display: 'block', marginBottom: '4px', color: '#8b949e', fontSize: '11px' }}>Symbol</label>
        <input
          value={symbol}
          onChange={e => setSymbol(e.target.value.toUpperCase())}
          style={{ width: '100%', padding: '8px', background: '#0d1117', border: '1px solid #21262d', color: '#e6edf3', fontSize: '13px' }}
        />
      </div>

      <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
        <div style={{ flex: 1 }}>
          <label style={{ display: 'block', marginBottom: '4px', color: '#8b949e', fontSize: '11px' }}>Side</label>
          <select
            value={side}
        ...
        <div style={{ flex: 1 }}>
          <label style={{ display: 'block', marginBottom: '4px', color: '#8b949e', fontSize: '11px' }}>Qty</label>
          <input
            type="number"
            value={qty}
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
        <div style={{ flex: 1 }}>
          <label style={{ display: 'block', marginBottom: '4px', color: '#8b949e', fontSize: '11px' }}>Type</label>
          <select
            value={type}
        ...
        {type === 'limit' && (
          <div style={{ flex: 1 }}>
            <label style={{ display: 'block', marginBottom: '4px', color: '#8b949e', fontSize: '11px' }}>Limit Price</label>
            <input
              type="number"
              step="0.01"
              value={price}
      ...
      {message && (
        <div style={{ marginTop: '10px', padding: '8px', background: '#161b22', borderRadius: '4px', fontSize: '11px' }}>
          {message}
        </div>
      )}
    </div>
